    ...
    useEffect(() => {
        const fetchData = async() => {
            ...
            const fetchPromises = keys.map(async(key) => {
                const cacheKey = `${'http://localhost/chinchinaes/api/controllers/endpoints/get/revista.php'}_${JSON.stringify(key)}`;
                const now = Date.now();
                ...
                try {
                    const response = await fetch('http://localhost/chinchinaes/api/controllers/endpoints/get/revista.php', {
                        method: "POST",
                        headers: {
