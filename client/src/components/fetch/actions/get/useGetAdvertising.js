    ...
    useEffect(() => {
        const fetchData = async() => {
            ...
            const fetchPromises = keys.map(async(key) => {
                const cacheKey = `${'http://localhost/chinchinaes/api/controllers/endpoints/get/advertising.php'}_${JSON.stringify(key)}`;
                const now = Date.now();
                ...
                try {
                    const response = await fetch('http://localhost/chinchinaes/api/controllers/endpoints/get/advertising.php', {
                        method: "POST",
                        headers: {
