...
if (isset($_SESSION['error_information'])) {
    $error = $_SESSION['error_information'];
    echo "<script type='text/javascript'>alert('$error');</script>";
    unset($_SESSION['error_information']);
};
...
<div class="section-zone">
    ...
    ?>
        ...
        <div class="container-table">
            <table>
                ...
                <tbody>
                    <?php
                    $sql = "SELECT * FROM info_dfc INNER JOIN view_ ON info_dfc.vie_id = view_.vie_id WHERE info_dfc.fai_id = '$fair_id'";
                    $query = mysqli_query($con, $sql);
                    if ($query && $row = $query->fetch_assoc()) {
                    ?>
                        <tr>
                            <td><?php echo $row['inf_id']; ?></td>
                            <td><?php echo $row['inf_title']; ?></td>
                            <td><?php echo $row['inf_description']; ?></td>
                            <td><?php echo $row['inf_general_objective']; ?></td>
                            <td><?php echo $row['vie_view']; ?></td>
                            <td>
                                <a href="./admin.php?p=fair&p2=introduction-edit&id=<?php echo $row['inf_id']; ?>"><img src="../../assets/img/edit.png" alt="Editar" class="image-table" loading="lazy"></a>
                                <a href="../../php/admin/delete_information.php?id=<?php echo $row['inf_id']; ?>"><img src="../../assets/img/delete.png" alt="Eliminar" class="image-table" loading="lazy"></a>
                            </td>
                        </tr>
                    <?php
                    } else {
        ...
        <a href="./admin.php?p=fair&p2=introduction-add" class="add-table" aria-label="Agregar nueva información">+</a>
    <?php } ?>


    <?php
    if ($_GET['p2'] === 'introduction-add') {
    ?>
        <form class="form-form" method="post" action="../../php/admin/add_information.php" enctype="multipart/form-data">
            ...
            <div class="group-form">
                <textarea name="general_objective" class="input-form" id="general_objective" placeholder="Objetivo General" required></textarea>
                <label for="general_objective" class="label-form">Objetivo General</label>
            </div>
    ...
    <?php
    if ($_GET['p2'] === 'introduction-edit') {
        if (isset($_GET['id'])) {
            $id = $_GET['id'];
            $sql = "SELECT * FROM info_dfc INNER JOIN view_ ON info_dfc.vie_id = view_.vie_id WHERE inf_id = '$id'";
            $query = mysqli_query($con, $sql);
            if ($query) {
                $row = $query->fetch_assoc();
    ?>
                <form class="form-form" method="post" action="../../php/admin/edit_information.php" enctype="multipart/form-data">
                    <h2 class="title-form">Editar Información</h2>

                    <input type="hidden" name="id" value="<?php echo $row['inf_id'] ?>">

                    <div class="group-form">
                        <input type="text" id="title_edit" name="title" placeholder="Título" value="<?php echo $row['inf_title'] ?>" class="input-form" required>
                        <label for="title_edit" class="label-form">Título</label>
                    </div>

                    <div class="group-form">
                        <textarea name="description" class="input-form" id="description_edit" placeholder="Descripción" required><?php echo $row['inf_description'] ?></textarea>
                        <label for="description_edit" class="label-form">Descripción</label>
                    </div>

                    <div class="group-form">
                        <textarea name="general_objective" class="input-form" id="general_objective_edit" placeholder="Objetivo General" required><?php echo $row['inf_general_objective'] ?></textarea>
                        <label for="general_objective_edit" class="label-form">Objetivo General</label>
                    </div>
